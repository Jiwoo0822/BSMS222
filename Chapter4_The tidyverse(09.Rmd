---
title: "Chapter4_The tidyverse(0915)"
output:
  html_document:
    df_print: paged
---
...
# answer- DNase, Formaldehyde, Orange 
```
## 4.3 Manipulation data frames
...
pander(murders <- mutate(murders, rank=rank(-rate)))
...
```{r}
library(pander)
pander(no_florida <- filter(murders, state != "Florida"))
pander(murders)
```
...
original data <e2>??select <e2>??filter 
...
```{r}
library(pander)
pander(murders <- mutate(murders, rate = total / population*100000, rank=rank(-rate)))
...
pander(filter(murders, region %in% c("Northeast", "West")&rate<1)%>%select(state,rate,rank))
