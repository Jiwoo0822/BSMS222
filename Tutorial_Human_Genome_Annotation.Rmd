...
library(dslabs)
library(tidyverse)
d = rtracklayer::import('gencode.v31.basic.annotation.gtf.gz') %>% as.data.frame()
```
...
```{r}
library(pander)
a <- d %>% group_by(feature_type) %>% count(feature_type)
pander(a)
...
```{r}
b <-d %>% group_by(source) %>% count(source)
pander(b)
```
## 2.5 do.call
...
d2 %>% filter(type == 'transcript') %>%
  ...
  unlist() %>%
  as.numeric() %>%
  quantile(na.rm = TRUE ,seq(0.25, 0.75, 0.25))

# answer - 1/1/2

c <- d2 %>% filter(type == 'transcript') %>%
  group_by(gene_id) %>% 
  count() %>%
  ungroup() %>% 
  top_n(1, n)
...
```{r}
d2 %>%group_by(transcript_type)%>%
  filter(transcript_type == 'protein_coding') %>%
  count() %>%
  pander()
d2 %>%group_by(transcript_type)%>%
  filter(transcript_type == 'miRNA') %>%
  count()%>%
  pander()
d2 %>% group_by(transcript_type)%>%
  filter(transcript_type == 'processed_pseudogene') %>%
  count() %>%
  pander()
```
...
```{r}
d2 %>% group_by(seqnames) %>%
  summarize(distribution = quantile(width, seq(0,1,0.25)))%>% pander()
```
...
```{r}
d2 %>% group_by(transcript_support_level, type) %>%
  filter(type == c("transcript"))%>%
  count() %>% pander()
```
...
```{r}
d2 %>% group_by(transcript_support_level, gene_type)%>%
  count() %>% pander()
```
3.From the first question, please count the number of transcript for each TSL by source.

```{r}
d2 %>% group_by(transcript_support_level, source)%>%
  count() %>% pander()
```
...
```{r}
gene <- data.frame(gene_id = d2$gene_id, gene_name = d2$gene_name, hgnc_id = d2$hgnc_id, gene_type =d2$gene_type, chromosome = d2$seqnames, start = d2$start, end = d2$end, strand = d2$strand, isHgnc = (is.na(d2$hgnc_id == "NA")), isCCDS = (is.na(d2$ccdsid == "NA")))
f <- head(gene)
pander(f)
```
...
```{r}
gene %>% group_by(gene_type, hgnc_id) %>% 
  count() %>% head() %>%pander()
```
...
```{r}
d2 %>% group_by(transcript_type) %>%
  filter(transcript_type == c("protein_coding")) %>%
  summarize(distribution = quantile(width, seq(0, 1, 0.25))) %>% pander()

d2 %>% group_by(transcript_type) %>%
  filter(transcript_type == c("lncRNA")) %>%
  summarize(distribution = quantile(width, seq(0, 1, 0.25))) %>% pander()
```
...
```{r}
d2 %>% group_by(seqnames) %>% 
  filter(seqnames == c("chrX", "chrY")) %>%
  filter(type == 'gene')%>%
  count() %>%
  pull(n) %>%
  mean()
d2 %>% group_by(seqnames) %>%
  filter(seqnames != c("chrX")) %>%
  filter(seqnames != c("chrY")) %>%
  filter(type == 'gene') %>%
  count() %>%
  pull(n) %>%
  mean()
