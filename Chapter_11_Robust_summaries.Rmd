---
title: "Chapter_11_Robust_summaries"
output:
  html_document:
    df_print: paged
---
...
\Delta by adding \Delta \times 500 to a single number. The median, defined as the value for which half the values are smaller and the other half are bigger, is robust to such outliers. No matter how large we make the largest point, the median remains the same.
...
```{r}
reported_heights %>% arrange(desc(height)) %>% top_n(10, height) %>% head(10) %>%pander()
```
